...
{
    class when_modifying_a_shift_on_a_schedule_with_a_endtime_thats_in_the_pass
    {
        ...
        private Because of = () =>
        {
            ...
            SavedShift.EndTime = DateTime.Now.AddMinutes(-20);
            Exception = Catch.Exception(() => Repository.ModifyShiftOnSchedule(SavedShift));
        };

        private It should_throw_an_exception = () =>
            {
                Exception.Message.ShouldEqual("You cannot modify a shift with a end time that's in the pass");
            };
    }
}
