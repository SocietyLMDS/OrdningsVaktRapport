...
{
    class when_renewing_an_employee_password_because_you_forgot_with_a_valid_email_address
    {
        ...
        private Because of = () =>
            {
                _forgotResponse = Repository.ForgotPassword(Employee.EmailAddress);
                _returnedEmployee = Repository.GetEmployeeById(Employee);
            };

        private It should_create_a_new_employee = () =>
            {
                _forgotResponse.ShouldEqual("Succeeded");
                _returnedEmployee.Password.ShouldEqual(_returnedEmployee.Password);
            };
    }
}
