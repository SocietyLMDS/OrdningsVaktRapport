...
{
    class when_modifying_a_need_on_a_customer_object_without_specifying_a_start_time
    {
        ...
        private Because Of = () =>
        {
            ...
            _savedNeed.NumberOfPersonalNeeded = "8";
            _exception = Catch.Exception(() => Repository.ModifyNeedOnCustomerObject(_savedNeed));
        };

        private It should_throw_an_exception = () =>
        {
            _exception.Message.ShouldEqual("You cannot modify a need without specifying a start time");
        };
    }
}
