...
{
    class when_modifying_a_need_on_a_customer_object_without_specifying_number_of_personnel_needed
    {
        ...
        private Because Of = () =>
        {
            ...
            _savedNeed.StartDateTime = DateTime.Now.AddMinutes(30);
            _savedNeed.EndDateTime = DateTime.Now.AddMinutes(40);
            _savedNeed.NumberOfPersonalNeeded = "";
            _exception = Catch.Exception(()=>Repository.ModifyNeedOnCustomerObject(_savedNeed));
        };

        private It should_throw_an_exception = () =>
            {
                _exception.Message.ShouldEqual("You cannot modify a need without specifying number of personnel need");
            };
    }
}
